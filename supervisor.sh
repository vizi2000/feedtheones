#!/bin/bash
# The Ones AI Feed — process supervisor (adapted from luna-news)
PROJECT_DIR="/home/vizi/feed.theones.io/luna-port"
...
start_flask() {
    if pgrep -f 'feed.theones.io/luna-port/backend/app.py' >/dev/null; then return; fi
...
case "${1:-supervise}" in
    ...
    stop)
        log "Stopping..."
        pkill -9 -f 'feed.theones.io/luna-port/backend/app.py' 2>/dev/null || true
        ;;
    restart)
        pkill -9 -f 'feed.theones.io/luna-port/backend/app.py' 2>/dev/null || true
        sleep 1; start_flask ;;
    status)
        echo '=== Flask ==='; pgrep -af 'feed.theones.io/luna-port/backend/app.py' || echo 'NOT RUNNING'
